...
Note: The @ApplicationPath("/api/v1") annotation is declared in SensorApplication.java as required by the JAX-RS specification.
...
However, there is a chance that race conditions will arise. A standard LinkedHashMap is not thread-safe and may corrupt its internal state if two requests arrive simultaneously and attempt to write to the same map. This could be solved by using ConcurrentHashMap or the synchronized keyword on write operations in a production system to fix this.
...
Hypermedia as the Engine of Application State is referred to as HATEOAS. This means that without hardcoding any URLs, API responses include links to related resources so that a client can browse the entire API by following those links, just like clicking links on a webpage.

In my implementation, the discovery endpoint at GET / returns links to /rooms and /sensors. Without needing external documentation, a client can locate everything by starting at the root.
...
Returning just IDs would reduce the size of the response and save bandwidth. The N+1 problem, however, would need the client to make N more requests in order to retrieve the information for each room. For instance, the client would require 101 requests in total if there were 100 rooms, which would significantly increase latency and server load.

Because it is far more efficient for common use scenarios, such as rendering a list in a user interface, I decided to deliver whole room objects in a single response in my solution. Pagination is the best option for really huge collections. When entire objects are returned, the client receives everything they require in a single round trip.
...
For a number of reasons, I found this approach was much better. Initially, it assigned a single, distinct task to each class: SensorResource is responsible for managing sensors, whereas SensorReadingResource is responsible for managing readings. Secondly, it makes the code easier to scale, I would just create a new class if I wanted to add /sensors/{id}/alerts later. Third, there is no need to re-validate anything because the sub-resource always runs in the correct sensor context because it automatically obtains the sensorId from the locator method. The code would have been extremely difficult to comprehend and maintain if everything had been contained in a single controller.
...
When a sensor is created with a roomId that does not exist, I decided to return HTTP 422 Unprocessable Entity rather than 404 because 404 would be misleading.

The requested URL endpoint does not exist, as shown by HTTP 404. There is absolutely no issue with the URL in this instance; POST /sensors exists and functions properly. The roomId in the JSON payload refers to a room that is not in the system, which is the source of the issue. RFC 4918 defines HTTP 422 for precisely this scenario, where the server is able to parse the request but is unable to process it due to semantically incorrect information. A developer would waste time believing their URL was incorrect if I returned 404. 

Using 422 tells them clearly that their URL was correct but the data inside the request was the problem.
...
In my implementation, I used a GlobalExceptionMapper to capture all Throwable and return a generic 500 message, ensuring that raw Java stack traces are never accessible to external API consumers.

This is crucial because stack traces expose a great deal of sensitive data. Initially, they disclose the precise versions of the framework, such as Jersey 2.41 and Grizzly, which attackers can search for known vulnerabilities in CVE databases. Secondly, they reveal package structures such as com.campus.sensor.resource.SensorResource and internal class names, which displays the application's internal architecture. Third, they make the method call sequence visible, which enables attackers to provide harmful inputs by displaying the precise flow of the business logic. 

Fourth, the names and versions of all third-party libraries are revealed. Fifth, an attacker can systematically map out the complete error handling code by purposefully causing errors and examining the traces.

All of this is prevented by my GlobalExceptionMapper, which returns nothing helpful to the external caller and logs the complete trace internally in the NetBeans console, where only I can view it.
...
Instead of manually putting Logger.info() statements within each resource function, I implemented logging using a JAX-RS filter named ApiLoggingFilter that implements both ContainerRequestFilter and ContainerResponseFilter.

I selected filters for a number of reasons. First guaranteed coverage: I don't have to remember to include logging in every method I build because the filter runs automatically on each and every request. I would undoubtedly forget it in some methods if I used manual logging. Logging has nothing to do with business logic and shouldn't be combined with it, according to the second separation of concerns. The filter takes care of observability independently, and my resource methods just deal with their own logic. Third consistency: each request is generated by the filter in precisely the same log format. Depending on how each method was coded, manual logging would result in various formats. 

Fourth, maintainability: I just need to update one file if I wish to update the log format. I would have to change each resource method if I used manual logging. Fifth, when the entire request processing chain is finished, the ContainerResponseFilter has access to the final HTTP status code, which I am unable to obtain from a resource method that is only aware of its own return value.
